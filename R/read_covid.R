...
#' @param filepath Character. The local file path of the downloaded dataset.
...
#'
#' @export
read_covid <- function(filepath, data = c("inputDB", "Output_5", "Output_10",
                                          "qualityMetrics"),
                                          return = c("data.frame", "data.table",
                                                     "tibble"), ...) {
        stopifnot(is.character(data), is.character(filepath),
                  length(filepath) == 1)
